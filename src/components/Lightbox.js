...
import { generatePlaceholderUrl, generateRawImageUrl, cld } from '../utils/cloudinary';
...
const Lightbox = ({ images, currentImageIndex, lightboxOpen, closeLightbox, nextImage, prevImage }) => {
    ...
    useEffect(() => {
        const handleKeyDown = (e) => {
            ...
            switch (e.key) {
                case 'ArrowLeft':
                    if (!first)
                        prevImage(); // Go to previous if not the first image
                    break;
                case 'ArrowRight':
                    if (!last)
                        nextImage(); // Go to next if not the last image
                    break;
                case 'Escape':
    ...
    if (!mainImageUrl) {
        ...
        return (_jsx("div", { className: "fixed inset-0 bg-black z-50 flex items-center justify-center text-white", children: "Error loading image URL." }));
    }
