...
export const generateImageUrl = (
  ...
  // Use auto face detection with subject fallback for gravity
  if (height) {
    image.resize(fill().width(width).height(height).gravity('auto:face'));
  } else {
    image.resize(fill().width(width).gravity('auto:face'));
  }
