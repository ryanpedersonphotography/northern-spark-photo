...
import { grayscale } from "@cloudinary/url-gen/actions/effect"; // Import grayscale effect explicitly
import { focusOn } from "@cloudinary/url-gen/qualifiers/gravity"; // For gravity qualifiers
 ...
 */
export const generatePlaceholderUrl = (publicId) => {
 ...
 */
export const generateRawImageUrl = (publicId) => {
    if (!publicId) {
 ...
 */
export const generateImageUrl = (publicId, width = 1200, 
// height, imageQuality, enableHDR are kept for signature compatibility but ignored below
height, imageQuality, // No longer used here
enableHDR // No longer used here
) => {
    // Basic validation: return empty string if publicId is missing
    if (!publicId) {
 ...
 */
export const generateSrcSet = (publicId, widths = [400, 800, 1200, 1600, 2000], // Default set of common widths
imageQuality // No longer used by generateImageUrl
) => {
    // Basic validation: return empty string if publicId is missing
    if (!publicId) {
...
export const optimizeCloudinaryUrl = (url, transformations = {}) => {
    ...
    return generateImageUrl(publicId, transformations.width || 1200, // Use provided width or default to 1200
